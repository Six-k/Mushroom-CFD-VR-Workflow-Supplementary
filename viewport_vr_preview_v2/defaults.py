...
def vr_defaults_create_default(session_state):
    ...
    ami = vr_defaults_action_add(am,
                                 VRDefaultActions.TOGGLE_VISIBILITY.value,
                                 ["/user/hand/right"],
                                 "view3d.vr_toggle_visibility",
                                 'PRESS',
                                 False,
                                 "",
                                 False,
    ...
    ami = vr_defaults_action_add(am,
                                 VRDefaultActions.SELECT_ITEM.value,
                                 ["/user/hand/right"],
                                 "view3d.vr_select_item",
                                 'PRESS',
                                 False,
                                 "",
                                 False,
...
def _vr_add_missing_actions(am, session_state):
    existing = {ami.name for ami in am.actionmap_items}

    am_name = am.name
    if am_name == VRDefaultActionmaps.DEFAULT.value:
        name = VRDefaultActions.TOGGLE_VISIBILITY.value
        if name not in existing:
            ami = vr_defaults_action_add(
                am, name, ["/user/hand/right"],
                "view3d.vr_toggle_visibility",
                'PRESS', False, "", False, 0.0, 0.0, 0.0, 'PRESS')
            if ami:
                _add_action_binding(ami, VRDefaultActionbindings.INDEX.value,
                    VRDefaultActionprofiles.INDEX.value, ["/input/b/click"], 0.3, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.OCULUS.value,
                    VRDefaultActionprofiles.OCULUS.value, ["/input/b/click"], 0.3, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.REVERB_G2.value,
                    VRDefaultActionprofiles.REVERB_G2.value, ["/input/thumbstick/click"], 0.3, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.VIVE_COSMOS.value,
                    VRDefaultActionprofiles.VIVE_COSMOS.value, ["/input/b/click"], 0.3, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.VIVE_FOCUS.value,
                    VRDefaultActionprofiles.VIVE_FOCUS.value, ["/input/b/click"], 0.3, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.VIVE.value,
                    VRDefaultActionprofiles.VIVE.value, ["/input/trackpad/click"], 0.3, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.WMR.value,
                    VRDefaultActionprofiles.WMR.value, ["/input/thumbstick/click"], 0.3, 'ANY', 'ANY')
            _register_action_with_runtime(am, ami, session_state)

        name = VRDefaultActions.SELECT_ITEM.value
        if name not in existing:
            ami = vr_defaults_action_add(
                am, name, ["/user/hand/right"],
                "view3d.vr_select_item",
            ...
            if ami:
                _add_action_binding(ami, VRDefaultActionbindings.INDEX.value,
                    VRDefaultActionprofiles.INDEX.value, ["/input/trigger/value"], 0.5, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.OCULUS.value,
                    VRDefaultActionprofiles.OCULUS.value, ["/input/trigger/value"], 0.5, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.REVERB_G2.value,
                    VRDefaultActionprofiles.REVERB_G2.value, ["/input/trigger/value"], 0.5, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.VIVE_COSMOS.value,
                    VRDefaultActionprofiles.VIVE_COSMOS.value, ["/input/trigger/value"], 0.5, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.VIVE_FOCUS.value,
                    VRDefaultActionprofiles.VIVE_FOCUS.value, ["/input/trigger/value"], 0.5, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.VIVE.value,
                    VRDefaultActionprofiles.VIVE.value, ["/input/trigger/value"], 0.5, 'ANY', 'ANY')
                _add_action_binding(ami, VRDefaultActionbindings.WMR.value,
                    VRDefaultActionprofiles.WMR.value, ["/input/trigger/value"], 0.5, 'ANY', 'ANY')
            _register_action_with_runtime(am, ami, session_state)


def _add_action_binding(ami, name, profile, component_paths, threshold, axis0_region, axis1_region):
    amb = ami.bindings.new(name, True)
    if amb:
        amb.profile = profile
        for path in component_paths:
...
def _register_action_with_runtime(am, ami, session_state):
    try:
        context = bpy.context
        ok = session_state.action_create(context, am, ami)
        if not ok:
            print(f"VR: failed to create action '{ami.name}'")
            return
        for amb in ami.bindings:
            session_state.action_binding_create(context, am, ami, amb)
    except Exception as e:
        print(f"VR: action runtime registration error: {e}")
...
def vr_ensure_default_actionmaps(session_state):
    ...
    if loaded:
        for am in session_state.actionmaps:
            _vr_add_missing_actions(am, session_state)
        return loaded
