...
def vr_create_actions(context: bpy.context):
    ...
    for am in session_state.actionmaps:
        ...
        action_set_ok = session_state.action_set_create(context, am)
        if not action_set_ok:
            print(f"VR: action set '{am.name}' exists, adding new actions only")
        ...
        for ami in am.actionmap_items:
            ...
            if not ok:
                print(f"VR: skipping action '{ami.name}' (already exists)")
                continue
            ...
            for amb in ami.bindings:
                ...
                if not ok:
                    print(f"VR: skipping binding '{amb.name}' for '{ami.name}' (already exists)")
